...
%ip=1:500;
%rng default
LB=[-1 -1 -1 -1 -1.1 -1 -1 ]; %lower bounds of variables
UB=[1 1 1 1 1 1 1 ]; %upper bounds of variables
...
ipi=[0.0554,0.1661,0.1661,0.0554,1.0000,-1.0784,0.6450,-0.1236];
theta1=[ipi(1),ipi(2),ipi(3),ipi(4)];
theta2=[ipi(5),ipi(6),ipi(7),ipi(8)];
...
%  end
 ...
 % pso algorithm-----------------------------------------------------end
 gbest;
 theta3=[gbest(1),gbest(2),gbest(3),gbest(4)];
theta4=[1,gbest(5),gbest(6),gbest(7)];

k1=filter(theta3,theta4,ip);

 fvalue=sum((k-k1).^2)/100;
 fff(run)=fvalue;
 rgbest(run,:)=gbest;
 disp(sprintf('--------------------------------------'));
end
